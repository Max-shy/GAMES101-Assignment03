...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    x_min = std::min(t.v[0].x(), std::min(t.v[1].x(), t.v[2].x()));//确定三角形x下边界
    x_max = std::max(t.v[0].x(), std::max(t.v[1].x(), t.v[2].x()));//确定三角形x上边界
    y_min = std::min(t.v[0].y(), std::min(t.v[1].y(), t.v[2].y()));//确定三角形y下边界
    y_max = std::max(t.v[0].y(), std::max(t.v[1].y(), t.v[2].y()));//确定三角形y上边界
